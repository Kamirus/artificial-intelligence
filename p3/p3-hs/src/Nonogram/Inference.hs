...
buildRow' n c offsets = if G.length row == n
  then row
  else row G.++ (G.generate (n - G.length row) $ const X)
 where
  f :: Int -> Int -> [Field]
  f offset blockLen =
    [ X | _ <- [1 .. offset] ] ++ [ Checked | _ <- [1 .. blockLen] ]
  row = G.fromList $ concat $ zipWith f offsets c

buildRow :: Int -> Constraint -> [Int] -> Row
buildRow n c offsets = G.create $ do
  row <- new n
...
allRows n c =
  [ buildRow n c off | off <- allOffsets additionalBlanks blocksNo ]
 where
  blocksNo         = length c
...
inferAllCommon' row c = case allValidRows row c of
...
inferRows nonogram = Nonogram rowC colC . (b //) <$> (zip [0 ..] <$> bulk)
 where
  ...
  rowC = rows nonogram
  colC = cols nonogram
  bulk = G.ifoldr (\i c acc -> (:) <$> inferAllCommon' (b ! i) c <*> acc)
                  (Right [])
                  rowC
...
transposeBoard =
  G.fromList
    . map G.fromList
    . transpose
    . G.foldr (\row acc -> G.toList row : acc) []

inferCols :: Nonogram -> Either String Nonogram
inferCols nonogram =
  Nonogram rowC colC . transposeBoard . (bT //) . zip [0 ..] <$> bulk
 where
  bT   = transposeBoard $ board nonogram
  rowC = rows nonogram
  colC = cols nonogram
  bulk = G.ifoldr (\i c acc -> (:) <$> inferAllCommon' (bT ! i) c <*> acc)
                  (Right [])
                  colC
...
inferMax n = inferStep n >>= aux n
 where
  ...
  aux old new | old == new = return new
